...
export const searchCars = onRequest({ cors: true, maxInstances: 10 }, async (request, response) => {
    try {
...
Responde ÚNICAMENTE con un objeto JSON válido con estos campos (deja en null lo que no se mencione):
{
  ...
  "maxYear": año máximo o null
        ...
        let criteriaText = geminiData?.candidates?.[0]?.content?.parts?.[0]?.text || "{}";
        ...
        const carsList = topCars
            .map((car, idx) => `${idx + 1}. ${car.make} ${car.model} ${car.year || ""} - ${car.mileage.toLocaleString()} km, ${car.fuel}, ${car.transmission}${car.location ? `, ubicado en ${car.location}` : ""}`)
            .join("\n");
        const responseText = topCars.length > 0
